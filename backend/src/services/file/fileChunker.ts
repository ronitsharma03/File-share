...
class FileChunker {
  static DEFAULT_CHUNK_SIZE = 16 * 1024 * 1024;

  getOptimizedFileChunks(fileSize: number): number {
  ...
  generateChunkMetaData(fileSize: number): FileMetaData[] {
    ...
    return Array.from({ length: chunkCount }, (_, i) => ({
      ...
      size:
        i === chunkCount - 1 ? fileSize - i * actualChunkSize : actualChunkSize,
      isLastChunk: i === chunkCount - 1,
    }));
  }

  getChunkMetaData(fileSize: number, chunkIndex: number): FileMetaData {
    const actualChunkSize = this.getOptimizedFileChunks(fileSize);
    const chunkCount = this.getNumberOfChunks(fileSize);
    ...
    return {
      ...
      size:
        chunkIndex === chunkCount - 1
          ? fileSize - chunkIndex * actualChunkSize
          : actualChunkSize,
